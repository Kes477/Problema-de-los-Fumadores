...
public class Fumador extends Proceso implements Runnable {
	...
	// Utiles
	private boolean fumadorSatisfecho;
	private int recursoIdentificado;
	
	// Espacio para alojar los recursos tomados de la mesa
	private int[] recursosObtenidos = new int[2];
	...
	public boolean mesaLlena() {
		if(mesa.recursosEnLaMEsa[0]!=0
		&& mesa.recursosEnLaMEsa[1]!=0 
	...
	* (al fumador no le interesa), por lo que ocupa un semáforo de control 
	...
	*/
	...
	{
		/**
		 * if: Si no tiene el recurso, y el campo está lleno,
		 * guarda su identificador, 
		 * lo agarra (con lo cual mesa.recursosColocados[i] = 0)
		 * y manda un mensaje.
		 */
	if(this.mesa.recursosEnLaMEsa[i]!=this.IDrecurso) {
		this.recursoIdentificado=this.mesa.recursosEnLaMEsa[i];  //Lo guarda en recursoIdentificado
	...
	*/
	public void fumar() {
		...
		* en dado caso la suma de los ID de los recursos debe ser igual a 6.
		*/ 		
		if(this.recursosObtenidos[0]+this.recursosObtenidos[1]+this.IDrecurso==6)
		{
    ...
    public void run() {
        try {
            ...
            System.out.println("- El fumador terminó de fumar.\n");    
                ...
                System.out.println("- El fumador con " + super.recursos[this.IDrecurso] + " está enojado.\n");
        	 } // fin if     	
        } // fin try
